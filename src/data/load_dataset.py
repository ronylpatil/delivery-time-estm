...
def extract_data(
    ...
    try:
        ...
        infologger.info(
            f"unable to load data from S3 bucket [check load_data()]. exc: {e}"
        )
    else:
...
def main() -> None:
    ...
    secret_file = home_dir + "/secrets.yaml"
    try:
        ...
        output_path = home_dir + params["load_dataset"]["raw_data"]
        pathlib.Path(output_path).mkdir(parents=True, exist_ok=True)
        data = extract_data(
            bucket=params["load_dataset"]["bucket"],
            key=params["load_dataset"]["filename"],
            access_key=sc_params["aws_access_key_id"],
            secret_key=sc_params["aws_secret_access_key"],
        )
        save_data(
            data, output_path=output_path, file_name=params["load_dataset"]["filename"]
        )
